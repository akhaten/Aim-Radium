...
int main( int argc, char* argv[] ) {
    ...
    for(int x = 0; x < 2; ++x){
        for(int z = 0; z < 2; ++z){
            ...
            auto light_entity = entity_manager->createEntity();
            auto light = new Ra::Engine::Scene::PointLight(light_entity);
            light->setColor(Ra::Core::Utils::Color::White());
            light->setPosition(token);
            viewer->getRenderer()->addLight(light);
        }
    }
    ...
    auto anime_datas = new AnimationDatas(5_ra, 1_ra, 1, Ra::Core::Vector3(1_ra, 0_ra, 0_ra));
